...
public class Form3DAOImpl implements Form3DAO {
	JDBCServiceLocator db = new JDBCServiceLocator();
	
	private Logger log = Logger.getLogger(Form3DAOImpl.class); 
	@Override
	public void saveFrom3Table1(Form3Model form3) throws SQLException {
		...
		sql.append("insert into JDA_FORM3_CTRL (FORM_NO, FORM_NAME, STATUS, UPDATE_BY) values (?, ?, ?, ?)");
		log.debug("sql >> " + sql.toString());
		try {
			ps = conn.prepareStatement(sql.toString());
			int parameterIndex = 1;
			ps.setString(parameterIndex++, form_no);
			ps.setString(parameterIndex++, form3.getForm_name());
			ps.setString(parameterIndex++, form3.getForm_status());
			ps.setString(parameterIndex++, form3.getUpdate_by());
			rs = ps.executeQuery();
	...
	public boolean UpdateFrom3Table(Form3Model form3) throws SQLException {
		...
		try {
			...
			sql.append("UPDATE JDA_FORM3_CTRL SET STATUS = ?, UPDATE_BY = ? ");
			sql.append("WHERE FORM_NO = ? ");
			log.debug("updateRole JDA_FORM1_CTRL >>> " + sql.toString());
			ps = conn.prepareStatement(sql.toString());
			int seq=1;
			log.debug("form1.getForm_no() >> "+form3.getForm_no());
			
			ps.setString(seq++, form3.getForm_status());
			ps.setString(seq++, form3.getUpdate_by());
			ps.setString(seq++, form3.getForm_no());
