...
public class EJDAM020Action extends AbstractAction {

	private Logger log = Logger.getLogger(EJDAM020Action.class);
...
//		form3.setForm_no(formNo);
		try{
			...
			Form1Model form1 = ejdam010Action.setValueModel("3","P",iuser);
...
//			dao.UpdateFrom3Table(form3);
			...
			transactionLogModel.setMenuId("M020");
			transactionLogModel.setTranAction("ADD");
			transactionLogModel.setDescription("Update EJDA Table 3 Form 3");
...
//		log.debug("ejdaMethod = "+getRequest().getParameter("ejdaMethod"));
		...
		try{
			...
			form3Bean.setActionName("EJDAM020");
	...
	private String setSQL(Form1Model form3Cri){
		...
		try{
			...
			sql.append("  WHERE JDA_TYPE = '3' AND DOC_STATUS = 'S' ");
