...
public class EJDAM015Action extends AbstractAction {

	private Logger log = Logger.getLogger(EJDAM015Action.class);
...
//			dao.saveFrom1Table1(form2);
	...
	private boolean doSubmitButton() throws Exception {
		...
		Vector vcDocAttach = ejdam010Action.setValueDocumentAttach("2", "S");
		...
		try{
			...
			transactionLogModel.setMenuId("M015");
			transactionLogModel.setTranAction("UPD");
			transactionLogModel.setDescription("Update EJDA Table 2 Form 2");
...
//		log.debug("ejdaMethod = "+getRequest().getParameter("ejdaMethod"));
	...
	private String setSQL(Form1Model form2Cri){
		...
		try{
			...
			sql.append(" WHERE JDA_TYPE = '2' AND DOC_STATUS = 'A' ");
		}catch(Exception e){
			e.printStackTrace();
		}
